...
def test_read(sut):
    with sut.ros1() as ros:
        config = ros.roslaunch.read(
            "pickplace_playground.launch", package="fetch_gazebo"
        )
        assert (
            "/ros_ws/src/fetch_gazebo/fetch_gazebo/models:"
            in config.envs["GAZEBO_MODEL_PATH"].value
        )

        assert (
            config.params["/arm_controller/follow_joint_trajectory/type"].value
            == "robot_controllers/FollowJointTrajectoryController"
        )
        assert (
            config.params["/gazebo/bellows_joint/position/i_clamp"].value
            == 0.0
        )
...
def test_eval_args_in_launch_file(sut):
    with sut.ros1() as ros:
        config = ros.roslaunch.read(
            "spawn_husky.launch", package="husky_gazebo"
        )
        actual_node_names = {node.name for node in config.nodes}
        expected_node_names = {
...
@pytest.mark.skip(
    reason="This is currently failing due to an Xacro parsing issue"
)
@pytest.mark.parametrize("sut", ["fetch"], indirect=True)
def test_remappings(sut):
    with sut.ros1() as ros:
        launch = ros.roslaunch(
            "pickplace_playground.launch",
            package="fetch_gazebo",
            node_to_remappings={
                "gazebo": [("/gazebo/model_states", "/funkybits")]
            },
        )
...
def test_locate_with_package_ros2(sut):
    actual_path = sut.ros2.launch.locate(
        "cartographer.launch.py", package="turtlebot3_cartographer"
    )
...
def test_launch_ros2(sut):
    actual_controller_command = sut.ros2.launch.launch(
        "cartographer.launch.py", package="turtlebot3_cartographer"
    ).popen.args
    expected_controller_command = (
...
def test_launch_with_full_path(sut):
    actual_controller_command = sut.ros2.launch.launch(
        "/ros_ws/src/turtlebot3/turtlebot3_cartographer/launch/cartographer.launch.py",
