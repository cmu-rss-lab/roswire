...
from ..common.source import extract_sources_from_cmake, NodeSourceInfo, PackageSourceExtractor
...
class ROS2PackageSourceExtractor(PackageSourceExtractor):
    ...
    @classmethod
    def for_app_instance(cls, app_instance: AppInstance) -> "ROS2PackageSourceExtractor":
    ...
    def extract_source_for_package(self, path_to_package: str) -> t.Mapping[str, NodeSourceInfo]:
        ...
        if self._files.isfile(setuppy_path):
            logger.error("Do not know how to process ROS2 packages with setup.py yet.")
            return {}

        logger.error(f"There is no package information inside {path_to_package}. Is it a package soure directory?")
        return {}
