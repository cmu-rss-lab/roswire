...
class ROS1LaunchManager(ROSLaunchManager):
    ...
    def read(
        ...
        *,
        ...
        reader = ROS1LaunchFileReader(shell=self._shell, files=self._files)
    ...
    def locate(self, filename: str, *, package: Optional[str] = None) -> str:
        ...
        logger.debug(
            f"determing location of launch file [{filename}]"
            f" in package [{package}]"
        )
        command = f"rospack find {shlex.quote(package)}"
        try:
            package_path = self._shell.check_output(
    ...
    def launch(
        ...
        *,
        ...
        if node_to_remappings or launch_prefixes:
            ...
            if launch_prefixes:
                launch_config = launch_config.with_launch_prefixes(
                    launch_prefixes
                )

            if node_to_remappings:
                logger.debug(
                    "adding remappings to launch config: "
                    f"{node_to_remappings}"
                )
                launch_config = launch_config.with_remappings(
                    node_to_remappings
                )
                logger.debug(
                    "added remappings to launch config: " f"{launch_config}"
                )
