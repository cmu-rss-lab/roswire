...
from ...common.launch.rosparam import (
    load_from_yaml_string as load_rosparam_from_string,
)
...
Loader = Callable[
    ["ROS1LaunchFileReader", LaunchContext, LaunchConfig, ET.Element],
...
class ROS1LaunchFileReader(LaunchFileReader):
    ...
    def for_app_instance(cls, app_instance: "AppInstance") -> LaunchFileReader:
        return ROS1LaunchFileReader(
            shell=app_instance.shell, files=app_instance.files
        )
    ...
    def _load_tags(
        self, ctx: LaunchContext, cfg: LaunchConfig, tags: Sequence[ET.Element]
    ) -> Tuple[LaunchContext, LaunchConfig]:
    ...
    @tag("group", ["ns"])
    def _load_group_tag(
        self, ctx: LaunchContext, cfg: LaunchConfig, tag: ET.Element
    ) -> Tuple[LaunchContext, LaunchConfig]:
    ...
    def _load_param_tag(
        ...
        if command is not None:
            logger.debug(
                f"obtaining value for parameter [{name}] "
                f"via command:\n{command}"
            )
        ...
        cfg = cfg.with_param(
            name=fullname, typ=typ, value=value, command=command
        )

        return ctx, cfg

    @tag("rosparam", ["command", "ns", "file", "param", "subst_value"])
    def _load_rosparam_tag(
        self, ctx: LaunchContext, cfg: LaunchConfig, tag: ET.Element
    ) -> Tuple[LaunchContext, LaunchConfig]:
    ...
    def _load_node_tag(
        ...
        respawn_delay = self._read_optional_float(
            tag, "respawn_delay", ctx, 0.0
        )
    ...
    def _load_arg_tag(
        ...
        ctx = ctx.with_arg(name=name, value=value, default=default, doc=doc)
        return ctx, cfg
    ...
    def _read_namespace(
        self, ctx: LaunchContext, tag: ET.Element
    ) -> Optional[str]:
    ...
    def _read_optional_bool(
        self, elem: ET.Element, attrib: str, ctx: LaunchContext, default: None
    ) -> Optional[bool]:
        ...

    @overload
    def _read_optional_bool(
        self, elem: ET.Element, attrib: str, ctx: LaunchContext, default: bool
    ) -> bool:
        ...
    ...
    @overload
    def _read_optional_float(
        self, elem: ET.Element, attrib: str, ctx: LaunchContext, default: float
    ) -> float:
        ...
    ...
    def _read_optional(
        self, elem: ET.Element, attrib: str, ctx: LaunchContext
    ) -> Optional[str]:
    ...
    def _resolve_args(self, s: str, ctx: LaunchContext) -> str:
        ...
        resolver = ArgumentResolver(
            shell=self._shell, files=self._files, context=resolve_ctx
        )
    ...
    def locate_node_binary(self, package: str, node_type: str) -> str:
        ...
        logger.debug(
            f"locating binary for node_type [{node_type}] "
        ...
        if not path:
            ...
            command = "rospack find " f"{shlex.quote(package)}"
            try:
                package_dir = shell.check_output(
            ...
            if files.isfile(path_in_scripts_dir) and files.access(
                path_in_scripts_dir, os.X_OK
            ):
                path = path_in_scripts_dir
            elif files.isfile(path_in_nodes_dir) and files.access(
                path_in_nodes_dir, os.X_OK
            ):
